// filepath: f:\oupountest\lib\features\business\logic\business_registration_cubit.dart
...
class BusinessRegistrationCubit extends Cubit<BusinessRegistrationState> {
  ...
  bool get hasActivityBeenSelected => _hasActivityBeenSelected;
  ...
  // Set selected business activity
  void setSelectedBusinessActivity(String activityId) {
    try {
  ...
  void toggleCategory(String categoryId) {
    ...
    try {
      // تحقق من صحة معرف الفئة قبل الاستمرار
      bool categoryExists = _selectedActivity?.categories.any((cat) => cat.id == categoryId) ?? false;
      if (!categoryExists) {
        print('Warning: Category ID $categoryId not found in current activity!');
      }
      
      // نقوم بإنشاء نسخة جديدة من القائمة لضمان إعادة بناء واجهة المستخدم
      List<String> newSelectedCategories = List<String>.from(_selectedCategories);
      
      if (newSelectedCategories.contains(categoryId)) {
        ...
        if (_selectedActivity != null) {
          try {
            ...
            print('Error finding category when removing services: $e');
          }
        }
      } else {
        ...
        if (!newSelectedCategories.contains(categoryId)) {
          ...
          print('WARNING: Category was already in the selected list!');
        }
      }
      
      // نحدّث القائمة الأصلية بالقائمة الجديدة
      _selectedCategories = newSelectedCategories;
      
      print('Selected categories after toggle: ${_selectedCategories.length}');
      print('Selected categories IDs: $_selectedCategories');
      
      // نقوم بإصدار حدث تم تحديث الحالة بنفس الحالة الحالية لإجبار Flutter على إعادة البناء
      if (_selectedActivity != null) {
        print('Re-emitting BusinessActivitySelected after category toggle with activity: ${_selectedActivity!.nameAr}');
        
        // إنشاء نسخة من الكائن الحالي سيجبر Flutter على اعتبار الحالة قد تغيرت
        // حتى لو كان المحتوى متطابق، لأن المرجع سيكون مختلفاً
        final updatedActivity = BusinessActivityModel(
          id: _selectedActivity!.id,
          nameEn: _selectedActivity!.nameEn,
          nameAr: _selectedActivity!.nameAr,
          categories: _selectedActivity!.categories,
        );
        
        // إصدار حدث تحديث الحالة بنسخة جديدة من الكائن
        emit(BusinessActivitySelected(updatedActivity));
      } else {
        // Fallback to the general state if no activity is selected (shouldn't happen)
        print('WARNING: No selected activity during category toggle!');
        emit(BusinessActivitiesLoaded(_businessActivities));
      }
    } catch (e) {
  ...
  void toggleService(String serviceId) {
    print('Toggle service with ID: $serviceId');
    
    try {
      // إنشاء نسخة جديدة من قائمة الخدمات المحددة
      List<String> newSelectedServices = List<String>.from(_selectedServices);
      
      if (newSelectedServices.contains(serviceId)) {
        print('Removing service: $serviceId');
        newSelectedServices.remove(serviceId);
      } else {
      ...
      _selectedServices = newSelectedServices;
      
      print('Selected services after toggle: ${_selectedServices.length}');
      print('Selected services IDs: $_selectedServices');
      
      // Similar to category toggle, re-emit the selected activity state to refresh the UI
      if (_selectedActivity != null) {
        print('Re-emitting BusinessActivitySelected after service toggle');
        
        // إنشاء نسخة جديدة من الكائن لضمان تحديث الواجهة
        final updatedActivity = BusinessActivityModel(
          id: _selectedActivity!.id,
          nameEn: _selectedActivity!.nameEn,
          nameAr: _selectedActivity!.nameAr,
        ...
        print('No activity selected, falling back to BusinessActivitiesLoaded state');
        emit(BusinessActivitiesLoaded(_businessActivities));
      }
    } catch (e) {
  ...
  List<ServiceModel> getAvailableServices() {
    ...
    // تتبع أكثر تفصيلاً للفئات المحددة
    for (var categoryId in _selectedCategories) {
      try {
        final category = _selectedActivity!.categories.firstWhere(
          (cat) => cat.id == categoryId,
        );
        print('Selected category: ${category.categoryAr} (${category.id}), Services: ${category.services.length}');
      } catch (e) {
        print('Error finding category with ID $categoryId: $e');
      }
    }
    ...
    // طباعة تفاصيل الخدمات المتاحة لمساعدة التصحيح
    if (services.isNotEmpty) {
      print('Available services details:');
      for (var i = 0; i < services.length; i++) {
        print('  $i. ${services[i].serviceAr} (${services[i].id})');
      }
    }
